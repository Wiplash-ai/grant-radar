...
export function DeveloperPage() {
    ...
    <main>
      ...
      <div className="docs-layout">
        ...
        <article className="docs-content">
          ...
          <section id="endpoints"><div className="docs-label"><BookOpen size={16}/> Endpoints</div><h2>The complete v1 surface</h2><div className="endpoint-list"><div><code>GET /v1/grants</code><span>Search, filter, sort, facet, and paginate current opportunities.</span></div><div><code>GET /v1/grants/:key</code><span>Retrieve one complete opportunity briefing by key, Grants.gov ID, or opportunity number.</span></div><div><code>POST /v1/matches</code><span>Rank live opportunities against an organization profile with explainable reasons and cautions.</span></div><div><code>GET /v1/changes</code><span>Poll additions, material updates, and removals observed during daily catalog refreshes.</span></div><div><code>GET /v1/grants/:key/checklist</code><span>Generate a source-linked application-readiness checklist.</span></div><div><code>GET /v1/meta</code><span>Inspect catalog coverage, official sources, and refresh metadata.</span></div><div><code>GET /openapi.json</code><span>Download the machine-readable OpenAPI 3.1 contract.</span></div></div></section>
  ...
  "meta": { "facets": { "agencies": [], "fundingCategories": [] } }
          ...
          <section id="agents"><div className="docs-label"><Braces size={16}/> Agent integration</div><h2>Give an agent the official Grant Grinder skill</h2><p>The public skill teaches Codex and other compatible agents how to authenticate, search efficiently, follow pagination, retrieve complete records, cite official sources, and avoid exposing API keys.</p><div className="docs-actions"><a className="briefing-action" href={appPath("/skills/grant-grinder-api/SKILL.md")}>Download SKILL.md <ArrowUpRight size={15}/></a><a className="briefing-action secondary" href={appPath("/skills/grant-grinder-api/references/api.md")}>Agent API reference <ArrowUpRight size={15}/></a></div></section>

          <section id="usage"><div className="docs-label"><ShieldCheck size={16}/> Usage and attribution</div><h2>Present the data honestly</h2><ul className="docs-checklist"><li><CheckCircle2/>Link users to each record’s <code>officialUrl</code>.</li><li><CheckCircle2/>Treat the government notice as controlling for eligibility, dates, and application rules.</li><li><CheckCircle2/>Treat match scores and generated checklists as organizational aids, not eligibility decisions.</li><li><CheckCircle2/>Do not imply Wiplash.ai or Grant Grinder is a government agency.</li><li><CheckCircle2/>Cache responsibly and use the change feed when freshness matters.</li></ul></section>
        </article>
      </div>
    </main><SiteFooter /></div>;
}
  ...
  return <div className="page-shell policy-page"><SiteHeader />
    <main>
      ...
      <article className="policy-content">
        ...
        <section><span>02</span><div><h2>Information you provide</h2><p>The public website does not require an account to search. If you create a funding desk, we process your name, email address, securely hashed password, favorites, saved search criteria, and recent search history so those features can sync across devices. If you contact support, we process your email address and message. API customers may provide account, subscriber, and support information through Wiplash.ai or an API marketplace.</p></div></section>
        ...
        <section><span>04</span><div><h2>How the browser extension works</h2><p>When you click its toolbar button, the extension inserts a packaged, draggable Grant Grinder search widget into the active ordinary webpage. It uses temporary <code>activeTab</code> access only after that click and does not request persistent access to arbitrary websites. If you deliberately select text and choose the Grant Grinder context-menu command, only that phrase is submitted as a search term. On a first-party Grant Grinder opportunity page, the opportunity identifier visible in the URL can be used to request similar opportunities. The extension does not read full pages, forms, passwords, or unrelated tabs, and it does not store a paid API key or execute remote code.</p></div></section>
        ...
        <section><span>08</span><div><h2>Cookies and local storage</h2><p>The public web app does not currently set advertising or analytics cookies. Signed-in funding desks use a secure HTTP-only session cookie. The extension cannot read or copy that cookie. It stores the configured Grant Grinder app URL, widget position, recent search text, status, and sort preference in browser storage. When you are already signed in on a first-party Grant Grinder page, the extension can request your name or email, favorite opportunities, and saved searches through that page; a local copy can be retained for up to 15 minutes. Your browser vendor can sync eligible extension settings according to your browser account settings.</p></div></section>
        <section><span>09</span><div><h2>Retention</h2><p>Funding-desk account information, favorites, saved searches, and recent search history remain until you remove them or request account deletion. Extension settings remain until you change them, clear browser storage, or remove the extension. API account and entitlement records remain while access is active and as needed for billing, security, dispute resolution, and legal obligations. Infrastructure and marketplace logs follow those providers’ configured retention schedules.</p></div></section>
        <section><span>10</span><div><h2>Security</h2><p>Production traffic uses HTTPS. Funding-desk passwords are stored as salted scrypt hashes and sessions use HTTP-only same-site cookies. Commercial API keys are accepted in request headers and should be stored as secrets. We limit access to operational systems and use source-attributed public government data. No system is perfectly secure, so please report a suspected issue to support@wiplash.ai.</p></div></section>
        <section><span>11</span><div><h2>Your choices and rights</h2><p>You can search the public site without an account, remove favorites and saved searches, sign out, remove the extension, clear its storage, rotate or revoke a commercial API key, and contact us to request access, correction, or deletion of personal information we control. Depending on where you live, you may also have rights to object, restrict processing, receive a portable copy, or appeal a rights decision.</p></div></section>
